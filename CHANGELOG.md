...
Toutes les modifications notables de Daycove.

Le numéro de version suit le format **YYYY.MM.xx** (année.mois.compteur mensuel, remis à 01 chaque mois). En interne, la version est stockée au format semver `YYYY.M.x` (évite les zéros non significatifs), affichée au format `YYYY.MM.xx` dans l'application.
